...
@section('content')
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 animate-in">
        ...
        <div class="bg-white rounded-xl p-4 shadow-sm border border-terracotta-200">
            ...
            <p class="mt-2.5 text-xl font-bold text-cream-900">Rp{{ number_format($kpi['belum_dibayar'], 0, ',', '.') }}</p>
    ...
    <div class="mt-5 flex items-center justify-between gap-3 animate-in" style="animation-delay: 0.05s">
        <button onclick="generateTagihan()" id="btn-generate"
            class="flex items-center gap-2 bg-sage-700 text-white text-xs font-medium px-4 py-2.5 rounded-full hover:bg-sage-800 hover:shadow-md active:scale-[0.98] transition-all duration-150 ease-out">
            + Generate Tagihan Bulan Ini
        </button>

        <div class="flex gap-2">
            <select id="filter-status"
                class="text-xs font-medium text-cream-700 border border-cream-300 rounded-full pl-2 pr-6 py-2 bg-white focus:outline-none focus:ring-2 focus:ring-sage-400">
            ...
            <select id="filter-bulan"
                ...
                @foreach ($tagihans->pluck('bulan_tagihan')->unique()->sort()->reverse() as $bulan)
                    <option value="{{ $bulan }}">
                        {{ \Carbon\Carbon::createFromFormat('Y-m', $bulan)->translatedFormat('F Y') }}</option>
                @endforeach
            </select>
        </div>
    </div>
    ...
    <div class="mt-5 bg-white rounded-xl border border-cream-200 shadow-sm divide-y divide-cream-200 animate-in"
        ...
        @forelse ($tagihans as $t)
            ...
            <div class="tagihan-row flex items-center gap-4 px-5 py-3.5 hover:bg-cream-50 transition-colors duration-150"
                ...
                <p class="text-sm font-semibold text-cream-900 shrink-0">Rp{{ number_format($t->jumlah, 0, ',', '.') }}</p>
                <span
        ...
        <div id="filter-empty-state" class="hidden px-5 py-10 text-center">
            ...
            <p class="mt-1 text-xs text-cream-600">
                Coba ubah filter atau pilih status lain untuk melihat data tagihan.
            </p>
        </div>
    </div>
@endsection
...
@push('scripts')
    <script>
        ...
        function applyFilters() {
            ...
            const emptyState =
                document.getElementById('filter-empty-state');
